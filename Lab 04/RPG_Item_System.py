...
class Shield(Item):
    def __init__(self, name, defense, broken=False, shield_type="", rarity="common"):
        ...
        super().__init__(name, description, rarity)  # Pass description correctly
    ...
    def equip(self):
        if not self._ownership:  # If no owner, don't equip
            return ""
        self.active = True
        return f"{self.name} is equipped by {self._ownership}"
    ...
    def throw_away(self):
        self._ownership = ""  # Clear ownership
        self.active = False  # Shield cannot be used anymore
        return f"{self.name} is thrown away"
